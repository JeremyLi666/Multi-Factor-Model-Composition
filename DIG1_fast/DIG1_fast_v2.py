        ...
        def wrapper(*args, **kwargs):
            ...
            for i in range(retries):
                try:
                    ...
                    print(f"警告: 函数 {func.__name__} 发生网络异常 ({e})，正在进行第 {i + 1}/{retries} 次重试...")
                    if i < retries - 1:
                        time.sleep(delay * (2 ** i))  # 指数级退避
            print(f"错误: 函数 {func.__name__} 在 {retries} 次重试后仍失败，任务中断。")
            raise  # 重新抛出最后的异常
        return wrapper
    return decorator
...
def run_task(dataset_id, region, delay, instrumentType, universe, n_jobs, tag=None):
    ...
    # 封装登录和字段获取，使其具备断线重连能力
    @retry_on_exception(retries=5, delay=10)
    def login_and_get_datafields():
    ...
    try:
        group = login_and_get_datafields()
    except Exception as e:
        print(f"致命错误: 无法在多次重试后登录并获取字段，跳过任务。错误信息: {e}")
        return
...
def plan_dataset(dataset_id, region, delay, instrumentType, universe, n_jobs, tag=None):
    ...
    try:
        ...
        print(f"致命错误: 无法在多次重试后登录并获取字段，跳过统计。错误信息: {e}")
        return {
...
def run_multi_datasets(dataset_ids, region, delay, instrumentType, universe, n_jobs, tag=None):
    ...
    for i in range(start_index, len(dataset_ids)):
        ...
        # 内部已经包含了 `@while_true_try_decorator`，所以这里不用再包一层
        run_task(
            dataset_id=ds,
            region=region,
            delay=delay,
